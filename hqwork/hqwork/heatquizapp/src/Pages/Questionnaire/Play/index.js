...
export function QuestionnairePlay(){
    ...
    useEffect(() => {
        if(survey){
            ...
            setQuestionAnswers(Questions.sort((a,b) => a.Order - b.Order).map((q) => []))

            setQuestionTextAnswers(Questions.sort((a,b) => a.Order - b.Order).map((q) => {
                const {Choices} = q
            ...
            setQuestionRangeAnswers(Questions.sort((a,b) => a.Order - b.Order).map((q) => {
                const {Choices} = q
    ...
    const controlTabClick = () => {
        ...
        addSurveyStatistic(data).then(r => handleResponse(r, api, "Submitted", 1))
    }
    ...
    const goToSurvey = () => {
        if(!surveyKey.trim()){
            setStartError("Please add a name or code")
            return
        }
            ...
            <div className="QuestionnaireStartBox">
                <Space className="QuestionnaireStartBoxInner" direction="vertical">
                    ...
                    <Space align="end" size={"small"}>
                        <div>
                            <small className="default-title">Please enter your name/code to start the survey</small>
                            <Input 
                            value={surveyKey} 
                            className="QuestionnaireStartInput" 
                            onChange={(v) => {
                                setSurveyKey(v.target.value)
                            }}
                            onKeyPress={(e) => {
                                if (e.key === "Enter") {
                                    goToSurvey()
                            ...
                            />
                            <small className="default-red">{startError}</small>
                        </div>
                        <Button 
