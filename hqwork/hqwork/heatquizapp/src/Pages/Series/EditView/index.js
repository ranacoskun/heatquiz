...
import { beautifyDate, beautifyNumber, goToQuestionnaireViewEdit, handleResponse } from "../../../services/Auxillary";
import {TrophyOutlined, EditOutlined, ApartmentOutlined, PlusOutlined, AreaChartOutlined, ClockCircleOutlined, NotificationOutlined, DeleteOutlined, GroupOutlined, SyncOutlined} from '@ant-design/icons';
...
import { AddEditQuestionnaireSeries } from "./AddEditQuestionnaireSeries";
...
export function SeriesEditViewPage(){
    ...
    const {flipQuestionnaireRepeatability} = useQuestionnaires()

    const {deassignQuestionnaireSeries} = useQuestionnaires()
    ...
    const [showAddEditQuestionnaire, setShowAddEditQuestionnaire] = useState(false)
    ...
    const actionList = (Series) => [
        ...
        {
            key: 'add_edit_questionnaire',
            label: 'Add/Edit questionnaire',
            icon: <GroupOutlined /> ,
            onClick: () => {
                setShowAddEditQuestionnaire(true)    
            }
        },
        Series.QuestionnaireRelation && 
        {
            key: 'remove_questionnaire',
            label: 'Remove questionnaire',
            icon: <GroupOutlined /> ,
            onClick: () => {
                const data = new FormData()

                data.append("SurveyId", Series.QuestionnaireRelation.QuestionnaireId)
        ...
        Series.QuestionnaireRelation && 
        {
            key: 'flip_questionnaire',
            label: 'Make questionnaire ' + (Series.QuestionnaireRelation.IsRepeatable ? "non-repeatable" : "repeatable"),
            icon: <SyncOutlined /> ,
            onClick: () => {
                const data = new FormData()
                ...
                flipQuestionnaireRepeatability(data).then(r => handleResponse(r, api, "Flipped successfully", 1, () => {
                    loadData()
                }))
            }
        },
    ...
    const renderQuestionnaire = () => {
        const {QuestionnaireRelation} = Series
        const {Questionnaire, IsRepeatable} = QuestionnaireRelation
            ...
            <div>
                <Col className="series-edit-view-info-card series-edit-view-info-other-card">
                    <div
                        ...
                        onClick={() => goToQuestionnaireViewEdit(Questionnaire)}
                    >
                        <Statistic
                            title={
                            <Space size="large">
                                <p>Questionnaire</p>
                                <small className="default-gray">Repeatable {' '}{IsRepeatable ? <span className="default-green">Yes</span> : <span className="default-red">No</span>}</small>
                            </Space>}
                            value={Code}
                            valueStyle={{fontSize:'16px', color:'#808080'}}
    ...
    const renderSeries = () => {
        ...
        const {Code, AddedByName, DateCreated, Elements, IsRandom, RandomSize, NumberOfPools, QuestionnaireRelation} = Series
            ...
            <div>
                ...
                >
                   ...
                   <Row
                    gutter={12}
                   >    
                        {QuestionnaireRelation && renderQuestionnaire()}
                        <Col className="series-edit-view-info-card series-edit-view-info-other-card">
    ...
    return(
        <PagesWrapper>
            ...
            <AddEditQuestionnaireSeries 
                open={showAddEditQuestionnaire}
                onClose={() => setShowAddEditQuestionnaire(false)}
