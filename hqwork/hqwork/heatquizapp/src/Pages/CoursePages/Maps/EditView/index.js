...
import { FixURL, beautifyDate, beautifyNumber, goToQuestionnaireViewEdit, handleResponse } from "../../../../services/Auxillary";
import { FilePdfOutlined, LinkOutlined, PictureOutlined, SyncOutlined, PlusOutlined, AimOutlined, PrinterOutlined,CustomerServiceOutlined, CommentOutlined, SelectOutlined, TrophyOutlined, EyeOutlined, ClockCircleOutlined, DashboardOutlined, CaretRightOutlined, EditOutlined, BlockOutlined, NodeExpandOutlined , DeleteOutlined, NodeIndexOutlined} from '@ant-design/icons';
...
import { AssignQuestionnaireToElement } from "./AssignQuestionnaireToElement";
import { useQuestionnaires } from "../../../../contexts/QuestionnaireContext";
import { AssignAudioToElement } from "./AssignAudioToElement";
...
export function MapEditView(){
    ...
    const {deassignQuestionnaireMapElement, flipQuestionnaireMapElementRepeatability} = useQuestionnaires() 
    ...
    const [showAssignQuestionnaire, setShowAssignQuestionnaire] = useState(false)
    ...
    const [showAssignAudioAttachment, setShowAssignAudioAttachment] = useState(false)
    ...
    const renderElements = (Elements) => {
        return(
            ...
            >
                ...
                dataSource={Elements}
                renderItem={(e, ei) => {
                    const {Title, PDFURL, ExternalVideoLink, QuestionSeries,
                         MapAttachment, Badges, CourseMapElementImages,
                         RequiredElement, Relations, Threshold, Background_Image,
                         QuestionnaireRelation, AudioURL} = e
                            ...
                            <Row gutter={[6, 6]}>
                                ...
                                <Dropdown
                                    menu={{
                                        ...
                                        {
                                            key:'edit_attachment',
                                            label:'Update questionnaire attachment',
                                            icon: <EditOutlined/>,
                                            onClick: () => {
                                                setSelectedElement(e)
                                                setShowAssignQuestionnaire(true)
                                            }
                                        },{
                                            ...
                                            onClick: () => {
                                                ...
                                                data.append("MapElementId", e.Id)

                                                deassignQuestionnaireMapElement(data).then(r => handleResponse(r, api, "Removed successfully", 1, () => {
                                                    loadData()
                                                }))
                                            }
                                        },{
                                            key:'flip_questionnaire_repeatability',
                                            label:'Flip repeatability',
                                            icon: <SyncOutlined />,
                                            onClick: () => {
                                                const data = new FormData()
                                                ...
                                                flipQuestionnaireMapElementRepeatability(data).then(r => handleResponse(r, api, "Flipped successfully", 1, () => {
                                                    loadData()
                                                }))
                                            }
                                        },
                                        {
                                            key:'edit_view_questionnaire',
                                            label:'View questionnaire',
                                            icon: <EditOutlined/>,
                                            onClick: () => {
                                                navigate('/questionnaire_edit_view/'+QuestionnaireRelation.Questionnaire.Id)
                                            }
                                        }]
                                    }}
                                >
                                    ...
                                    >
                                    <CommentOutlined className="default-gray"/>
                                    <p className="default-gray">{QuestionnaireRelation.Questionnaire.Code}</p>

                                    <Tooltip
                                        color="white"
                                        title={<p>Repeatable {<span className={QuestionnaireRelation.IsRepeatable ? "default-green" : "default-red"}>{QuestionnaireRelation.IsRepeatable ? "Yes" : "No"}</span>}</p>}
                                    >
                                        <SyncOutlined className={QuestionnaireRelation.IsRepeatable ? "default-green" : "default-red"} />
                                    </Tooltip>
                                    </Space>
                                </Dropdown>
                                </Col>:
                                ...
                                <Dropdown
                                    menu={{
                                        ...
                                        items:[{
                                            key:'update_audio',
                                            label:'Update audio attachment',
                                            icon: <EditOutlined />,
                                            onClick: () => {
                                                setSelectedElement(e)
                                                setShowAssignAudioAttachment(true)
                                            }
                                        },
                                        {
                                            key:'delete_audio',
                                ...
                                className="please-select-area please-select-area-special">
                                    <PlusOutlined className="default-gray default-smaller"/>
                                    <CustomerServiceOutlined  style={{color:'gray', fontSize:'large'}}/>
                                </div>}
                            </Row>
    ...
    return(
        <PagesWrapper>
            ...
            <AssignQuestionnaireToElement 
                open={showAssignQuestionnaire}
                onClose={() => setShowAssignQuestionnaire(false)}
            ...
            <AssignAudioToElement 
                open={showAssignAudioAttachment}
                onClose={() => setShowAssignAudioAttachment(false)}
