...
export function SeriesPlay({Code, onExitSeries, onFinishPlaySeries, mapKey, mapName, mapElementName}){
    ...
    const loadData = () => {
        ...
        getSeries(Code + "/" + currentPlayerKey)
        setSeriesElements([])
        setCurrentIndex(0)
        setStopTimer(false)
    ...
    const renderFBDDiagramQuestionFinalPage = (index) => {
        const element = playedElements[index]
    ...
    const renderFinalPage = () => {
        ...
        if(!finishedSurvey && QuestionnaireRelation){
            const {QuestionnaireId} = QuestionnaireRelation
            return(
                <div>
                    <PlayQuestionnaireInSeries 
                        id={QuestionnaireId}
                        onFinish={() => setFinishedSurvey(true)}
                        Key = {mapKey}
                        Series={Series}
                    />
                </div>
        ...
        const answersDisplayIndexer = (type, index) => {
            const comp = ({
                ...
                [FBD_QUESTION_PARAMETER]: (index) => renderFBDDiagramQuestionFinalPage(index),
