...
//Function to open a new window with for a certin series
export const goToSeriesPlay = (s) => {
  const {Code} = s

  goToPageRelativePath('series_play/'+Code)
}
...
export const goToMapEditView = (m) => {
  const {Id} = m

  goToPageRelativePath('edit_view_map/'+Id)
}
...
export const goToQuestionnairePlay = (s) => {
  const {Id} = s

  goToPageRelativePath('questionnaire_play/'+Id)
}


export const goToQuestionnaireViewEdit = (s) => {
  const {Id} = s

  goToPageRelativePath('questionnaire_edit_view/'+Id)
}
